...
function inferRole(...parts: (string | undefined)[]): DiagramRole {
  ...
  if (/\b(client|web|mobile|frontend|ui|dashboard|portal|browser)\b/.test(value)) return "client";
  if (/\b(load balancer|balancer|gateway|api gateway|ingress|edge|proxy)\b/.test(value)) return "balancer";
  if (/\b(queue|kafka|rabbitmq|pubsub|stream|event bus|broker)\b/.test(value)) return "queue";
  if (/\b(worker|consumer|processor|runner|job)\b/.test(value)) return "worker";
  if (/\b(database|postgres|mysql|cockroach|mongo|dynamo|storage|warehouse)\b/.test(value)) return "database";
  if (/\b(cache|redis|memcached)\b/.test(value)) return "cache";
  if (/\b(cloud|cdn|ai|llm|provider|s3|object storage|blob)\b/.test(value)) return "cloud";
  return "service";
}
...
function normalizeDiagramNode(item: RawDiagramNode | undefined, index: number): DiagramNode {
  ...
  const role: DiagramRole =
    typeof rawRole === "string" && DIAGRAM_ROLES.has(rawRole as DiagramRole)
...
function normalizeTechStackEntry(item: unknown): TechStackItem | null {
  const o = item as { layer?: string; name?: string; reason?: string };
...
function normalizeApiEntry(item: unknown): ApiSpec | null {
  ...
  const o = item as { name?: string; method?: string; path?: string; purpose?: string };
  const name = normalizeText(o?.name);
...
function normalizeArchitecture(rawArchitecture: RawArchitectureInput | undefined, raw: RawDesignInput): Architecture {
  const summary = normalizeText(
    rawArchitecture?.summary,
    normalizeText(raw.workingIdea, normalizeText(raw.idea, DEFAULT_IDEA)),
  );
