import { Component, lazy, Suspense, useCallback, useEffect, useMemo, useRef, useState, type ChangeEvent, type CSSProperties, type ErrorInfo, type ReactNode, } from "react";
...
function stressTestForm(): FormState {
    return {
        ...
        constraints: {
            ...
            customRequirements: "Domain: Logistics / RegTech · Pattern: Event-driven microservices + CQRS read models · Require: deterministic replay, exactly-once webhook strategy (document trade-offs), blue/green deploys, chaos hooks, DR targets RPO ≤ 5m / RTO ≤ 30m.",
        },
    };
}
...
class ErrorBoundary extends Component<{
    ...
    render() {
        ...
        return (<div style={{ minHeight: "100vh", background: "#f7f7f7", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
        <div style={{ maxWidth: 480, textAlign: "center" }}>
          ...
          <button type="button" onClick={() => this.setState({ error: null })} style={{ marginTop: 24, border: "1px solid #111", background: "transparent", padding: "10px 28px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
            Try again
          </button>
        </div>
      </div>);
...
function DiagramSkeleton() {
    return (<div className="h-full rounded-2xl border border-gray-200 bg-white p-5">
...
function Wordmark({ inverted = false, onClick }: {
    inverted?: boolean;
    onClick?: () => void;
}) {
...
function ConstraintField({ label, value, onChange, placeholder }: {
    label: string;
    value: string;
...
function NewDesignScreen({ onStart, onBack, }: {
    onStart: (payload: {
        prompt: string;
    ...
    return (<div style={{ background: BG, minHeight: "100vh", color: DARK }}>
      ...
      <div className="mx-auto flex max-w-4xl items-center justify-between px-4 py-4 sm:px-6 sm:py-5" style={{ borderBottom: `1px solid ${BORD}` }}>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          {[1, 2, 3].map(n => (<div key={n} className="flex items-center gap-1.5 sm:gap-2">
              <div className="flex h-6 w-6 items-center justify-center text-xs font-bold transition-colors" style={{ background: step >= n ? DARK : "transparent", color: step >= n ? BG : MUTED, border: `1px solid ${step >= n ? DARK : BORD}` }}>
                {step > n ? "✓" : n}
              </div>
      ...
      <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 sm:py-10">
        ...
        {step === 1 && (<div>
            ...
            <div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {DOMAINS.map(d => (<button key={d.id} type="button" onClick={() => goStep2(d)} className="card-hover group border p-5 text-left transition-all" style={{
        ...
        {step === 2 && (<div>
            ...
            <div className="mt-6 sm:mt-8">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {PATTERNS.map(p => (<button key={p.id} type="button" onClick={() => setPattern(p)} className="card-hover border p-5 text-left transition-all" style={{ borderColor: pattern?.id === p.id ? DARK : BORD, background: pattern?.id === p.id ? DARK : BG }}>
            ...
            <div className="mt-6 sm:mt-8">
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                {SCALES.map(s => (<button key={s.id} type="button" onClick={() => setScale(s)} className="card-hover border px-4 py-4 text-left transition-all" style={{ borderColor: scale?.id === s.id ? DARK : BORD, background: scale?.id === s.id ? DARK : BG }}>
            ...
            <button type="button" onClick={goStep3} disabled={!step2Done} className="btn-outline mt-10 border px-8 py-3.5 text-sm font-semibold disabled:opacity-30" style={{ borderColor: DARK, color: DARK }}>
              Continue →
            </button>
          </div>)}
        ...
        {step === 3 && (<div>
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              {[domain?.label, pattern?.label, scale?.sub].filter(Boolean).map(t => (<span key={t} className="border px-3 py-1 text-xs font-semibold" style={{ borderColor: DARK, color: DARK }}>{t}</span>))}
            </div>

            <textarea value={prompt} onChange={(e: ChangeEvent<HTMLTextAreaElement>) => setPrompt(e.target.value)} rows={8} style={{ ...inp, marginTop: 20, minHeight: 180 }} placeholder={`Describe your ${domain?.label || "product"} idea. Include the key user journeys, must-have features, and anything that makes your use case specific...`}/>
            ...
            ].map((s, i) => (<button key={i} type="button" onClick={() => setPrompt(s)} className="border px-3 py-1.5 text-xs transition-colors hover:border-black" style={{ borderColor: BORD, color: MUTED }}>
                    {s.slice(0, 60)}…
                  </button>))}
              </div>
            </div>

            <button type="button" onClick={handleStart} disabled={!prompt.trim()} className="btn-outline mt-8 border px-8 py-3.5 text-sm font-semibold disabled:opacity-30" style={{ borderColor: DARK, color: DARK }}>
...
function HeroDiagram() {
    return (<svg viewBox="0 0 780 440" className="h-full w-full" role="img" aria-label="Architecture preview">
      <defs>
...
function HistoryCard({ entry, onOpen }: {
    entry: DesignHistoryEntry;
    onOpen: (e: DesignHistoryEntry) => void;
}) {
    return (<button type="button" onClick={() => onOpen(entry)} className="group w-full border-b p-5 text-left transition-colors hover:bg-[#f9f9f9]" style={{ borderColor: BORD }}>
...
function DashboardScreen({ user, history, onNewDesign, onOpenHistory, onLogout, onDeleteDesign: _onDeleteDesign, }: {
    ...
    return (<div style={{ background: BG, minHeight: "100vh" }}>
      ...
      <main className="mx-auto max-w-2xl px-8 py-16">
        ...
        {history.length > 0 && (<div className="mt-12">
        ...
        ].map((idea, i) => (<button key={i} type="button" onClick={() => onNewDesign(idea)} className="flex w-full items-center justify-between border-b px-5 py-4 text-left text-sm transition-colors hover:bg-[#f9f9f9] last:border-0" style={{ borderColor: BORD, color: DARK }}>
                <span>{idea}</span>
...
function AuthScreen({ onAuth, onBack, onForgotPassword, initialMode = "signup", }: {
    onAuth: (mode: "signup" | "login", fields: {
        name?: string;
        email: string;
        password: string;
    }) => Promise<boolean>;
    ...
    return (<div style={{ background: GRAY, minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
        ...
        <div style={{ background: BG, border: `1px solid ${BORD}`, padding: 36 }}>
          ...
          <div className="mt-6 space-y-4">
            ...
            <label className="block">
              ...
              <input type="password" value={fields.password} onChange={(e: ChangeEvent<HTMLInputElement>) => set("password", e.target.value)} placeholder="Min 6 characters" onKeyDown={(e) => e.key === "Enter" && void handleSubmit()} style={inp}/>
            </label>
            {mode === "login" && onForgotPassword && (<div className="text-right">
          ...
          <button type="button" onClick={handleSubmit} disabled={busy} className="mt-6 w-full py-3 text-sm font-semibold text-white transition-opacity disabled:opacity-60 hover:opacity-80" style={{ background: DARK }}>
            {busy ? "Please wait…" : mode === "signup" ? "Create account →" : "Sign in →"}
          </button>

          <p className="mt-5 text-center text-sm" style={{ color: MUTED }}>
            {mode === "signup" ? "Have an account? " : "No account? "}
            <button type="button" onClick={() => { setMode(m => m === "signup" ? "login" : "signup"); setError(""); }} className="font-semibold underline" style={{ color: DARK }}>
...
function ForgotPasswordScreen({ onBack, onContinueWithCode, }: {
    onBack: () => void;
        ...
        <div style={{ background: BG, border: `1px solid ${BORD}`, padding: 36 }}>
          ...
          {!done ? (<>
              <label className="mt-6 block">
                <span className="mb-1.5 block text-xs font-semibold uppercase tracking-wide" style={{ color: MUTED }}>Email</span>
                <input type="email" value={email} onChange={(e: ChangeEvent<HTMLInputElement>) => setEmail(e.target.value)} placeholder="you@example.com" style={inp} onKeyDown={(e) => e.key === "Enter" && void submit()}/>
              </label>
              <button type="button" onClick={submit} disabled={busy || !email.trim()} className="mt-6 w-full py-3 text-sm font-semibold text-white transition-opacity disabled:opacity-40 hover:opacity-80" style={{ background: DARK }}>
                {busy ? "Please wait…" : "Send reset code →"}
              </button>
            </>) : (<div className="mt-6 space-y-4">
              <p className="text-sm leading-relaxed" style={{ color: DARK }}>{message}</p>
              {devCode && (<div className="space-y-2 rounded border p-3" style={{ borderColor: BORD, background: "#fafafa" }}>
                  ...
                  <p className="w-full py-3 text-center font-mono text-2xl font-bold tracking-[0.35em]" style={{ ...inp, letterSpacing: "0.35em" }}>
                    {devCode}
                  </p>
                  <button type="button" onClick={() => onContinueWithCode(devCode)} className="btn-outline w-full border py-2.5 text-sm font-semibold" style={{ borderColor: DARK, color: DARK }}>
                    Enter code and new password →
                  </button>
                </div>)}
              {!devCode && (<button type="button" onClick={() => onContinueWithCode("")} className="btn-outline w-full border py-2.5 text-sm font-semibold" style={{ borderColor: DARK, color: DARK }}>
                  I have my code →
                </button>)}
...
function normalizePasswordResetInput(raw: string) {
    ...
    if (/^[a-f0-9]+$/i.test(v) && v.length > 6)
        return v.toLowerCase().slice(0, 64);
    return v.replace(/\D/g, "").slice(0, 6);
}
function ResetPasswordScreen({ code, onCodeChange, onBack, onDone, }: {
    code: string;
        ...
        <div style={{ background: BG, border: `1px solid ${BORD}`, padding: 36 }}>
          ...
          <div className="mt-6 space-y-4">
            <label className="block">
              ...
              <input type="text" inputMode={/^[a-f0-9]+$/i.test(code) && !/^\d+$/.test(code) ? "text" : "numeric"} autoComplete="one-time-code" maxLength={64} value={code} onChange={(e: ChangeEvent<HTMLInputElement>) => onCodeChange(normalizePasswordResetInput(e.target.value))} placeholder="000000" className={`font-mono ${code.length > 6 ? "text-sm tracking-normal" : "text-center text-lg tracking-[0.4em]"}`} style={{ ...inp, letterSpacing: code.length > 6 ? "normal" : "0.4em" }}/>
            ...
            <label className="block">
              ...
              <input type="password" value={password} onChange={(e: ChangeEvent<HTMLInputElement>) => setPassword(e.target.value)} placeholder="At least 6 characters" style={inp}/>
            </label>
            <label className="block">
              <span className="mb-1.5 block text-xs font-semibold uppercase tracking-wide" style={{ color: MUTED }}>Confirm password</span>
              <input type="password" value={confirm} onChange={(e: ChangeEvent<HTMLInputElement>) => setConfirm(e.target.value)} placeholder="Repeat password" style={inp} onKeyDown={(e) => e.key === "Enter" && void submit()}/>
            </label>
          </div>

          <button type="button" onClick={submit} disabled={busy} className="mt-6 w-full py-3 text-sm font-semibold text-white transition-opacity disabled:opacity-60 hover:opacity-80" style={{ background: DARK }}>
...
function LandingScreen({ onGetStarted, onSignIn }: {
    ...
    return (<div style={{ ...S, color: DARK }}>
      ...
      <nav style={{ ...S, borderBottom: `1px solid ${BORD}`, position: "sticky", top: 0, zIndex: 50 }}>
        <div className="mx-auto flex max-w-5xl items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-6">
            ...
            <button type="button" onClick={onGetStarted} className="btn-outline border px-5 py-2 text-sm font-semibold" style={{ borderColor: DARK, color: DARK }}>
              Get started
            </button>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-5xl">
          <div className="grid items-start gap-10 lg:grid-cols-2 lg:gap-16">
            <div>
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                <button type="button" onClick={onGetStarted} className="btn-outline border px-6 py-3 text-sm font-semibold sm:px-7 sm:py-3.5" style={{ borderColor: DARK, color: DARK }}>
                  Try it free →
                </button>
                <a href="#features" className="nav-link px-6 py-3 text-sm font-semibold sm:px-7 sm:py-3.5" style={{ color: MUTED }}>
                  See how it works ↓
                </a>
              </div>
              <div className="mt-8 grid grid-cols-3 border-t pt-6" style={{ borderColor: BORD }}>
                {[["8+", "Sections"], ["< 10s", "Per design"], ["3", "Export formats"]].map(([v, l], i) => (<div key={l} style={{ paddingRight: i < 2 ? 16 : 0, paddingLeft: i > 0 ? 16 : 0, borderRight: i < 2 ? `1px solid ${BORD}` : "none" }}>
        ...
        <div className="mx-auto max-w-5xl border-t pt-10 sm:pt-16" style={{ borderColor: BORD }}>
          <div className="grid items-start gap-10 lg:grid-cols-2 lg:gap-16">
            <div>
              ...
              <button type="button" onClick={onGetStarted} className="btn-outline mt-6 border px-6 py-3 text-sm font-semibold sm:mt-8 sm:px-7 sm:py-3.5" style={{ borderColor: DARK, color: DARK }}>
                Start for free →
              </button>
            </div>
            ...
            <div className="card-hover mt-4 lg:mt-0" style={{ border: `1px solid ${BORD}` }}>
              ...
              {OUTPUT_SAMPLES.map(({ label, text }) => (<div key={label} className="sample-row border-b px-5 py-3.5 last:border-0 sm:py-4" style={{ borderColor: BORD }}>
      ...
      <section id="process" style={S} className="px-4 py-12 sm:px-6 sm:py-20">
        ...
        ].map(([n, t, b]) => (<div key={n} className="step-row flex gap-5 border-b py-6 sm:gap-6 sm:py-8" style={{ borderColor: BORD }}>
      ...
      <section style={S} className="px-4 py-16 sm:px-6 sm:py-24">
        <div className="mx-auto max-w-3xl border-t pt-12 sm:pt-16" style={{ borderColor: BORD }}>
          ...
          <div className="mt-7 flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-center sm:gap-5">
            <button type="button" onClick={onGetStarted} className="btn-outline border px-8 py-3.5 text-sm font-semibold" style={{ borderColor: DARK, color: DARK }}>
              Get started free →
            </button>
            <button type="button" onClick={onSignIn} className="text-link text-sm" style={{ color: MUTED }}>
...
function OnboardingScreen({ form, signup, onPromptChange, onConstraintChange, onSignupChange, onSubmit, onBack, }: {
    form: FormState;
    ...
    return (<div className="min-h-screen px-4 py-8 md:px-6" style={{ background: P.paper, color: P.text }}>
        ...
        <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_400px]">
          ...
          <div className="rounded-3xl border border-gray-200 bg-white p-7 shadow-sm">
            ...
            <div className="space-y-5">
              <label className="block space-y-2">
                ...
                <Input as="textarea" rows={6} value={form.prompt} onChange={(e: ChangeEvent<HTMLTextAreaElement>) => onPromptChange(e.target.value)} placeholder="e.g. Build a SaaS tool for engineering teams to track on-call incidents, assign alerts, and generate post-mortems automatically." className="min-h-[160px] resize-none bg-gray-50 text-sm"/>
              </label>
          ...
          <div className="space-y-5">
            <div className="rounded-3xl border border-gray-200 bg-white p-7 shadow-sm">
              ...
              <button type="button" onClick={onSubmit} className="mt-6 w-full rounded-xl bg-gray-950 py-3.5 text-sm font-black text-white shadow-[0_6px_20px_rgba(15,23,42,0.16)] transition-all duration-200 hover:bg-blue-600">
                Open workspace →
              </button>
            </div>
            <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
              ...
              <ul className="mt-4 space-y-3">
                {["Architecture diagram with components and flows", "Functional + non-functional requirements", "Tech stack with reasoning for each choice", "API definitions and data schema", "Export to PDF, Word, or Markdown"].map((item) => (<li key={item} className="flex items-start gap-3 text-sm text-gray-700">
...
function WorkspaceScreen({ form, design, loading, exporting, hasDesign, showInsights, showExportMenu, metrics, user, designMeta, generationMeta, requireAuthForAi, onRequestSignIn, onPromptChange, onConstraintChange, onLoadStressTest, onGenerate, onReset, onExport, onToggleInsights, onToggleExportMenu, onCloseMenus: _onCloseMenus, onGoHome, onNewDesign, }: {
    form: FormState;
    design: NormalizedDesign;
    loading: boolean;
    exporting: string;
    ...
    return (<div className="workspace-root flex flex-col" style={{ background: GRAY }}>
      ...
      <header className="flex shrink-0 items-center justify-between border-b bg-white px-4 py-3" style={{ borderColor: BORD }}>
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          {(domainLabel || patternLabel) && (<div className="hidden items-center gap-1.5 md:flex">
        ...
        <div className="flex shrink-0 items-center gap-1.5">
          ...
          <button type="button" onClick={onNewDesign} className="btn-outline hidden border px-3 py-2 text-xs font-semibold sm:block" style={{ borderColor: BORD, color: DARK }}>
            + New design
          </button>

          <div className="relative hidden sm:block">
            <button type="button" disabled={!hasDesign || Boolean(exporting)} onClick={onToggleExportMenu} className="btn-outline border px-3 py-2 text-xs font-semibold disabled:opacity-40" style={{ borderColor: BORD, color: DARK }}>
              {exporting ? "…" : "Export"}
            </button>
            {showExportMenu && (<div className="absolute right-0 top-9 z-30 w-40 overflow-hidden border bg-white shadow-xl" style={{ borderColor: BORD }}>
                {[["md", "Markdown (.md)"], ["pdf", "PDF (.pdf)"], ["docx", "Word (.docx)"]].map(([fmt, label]) => (<button key={fmt} type="button" onClick={() => onExport(fmt as ExportFormat)} className="flex w-full px-4 py-2.5 text-sm transition-colors hover:bg-gray-50" style={{ color: DARK }}>
                    {label}
                  </button>))}
              </div>)}
          </div>
          ...
          <button type="button" onClick={onGenerate} disabled={loading} className="btn-outline border px-4 py-2 text-xs font-bold disabled:opacity-50" style={{ borderColor: DARK, color: DARK }}>
            {loading ? "…" : "Generate"}
          </button>

          <button type="button" onClick={onToggleInsights} className="btn-outline hidden border px-3 py-2 text-xs font-semibold sm:block" style={{ borderColor: showInsights ? DARK : BORD, background: showInsights ? DARK : BG }}>
            <span style={{ color: showInsights ? BG : DARK }}>Insights</span>
          </button>

          <div className="mx-0.5 hidden h-4 w-px sm:block" style={{ background: BORD }}/>

          <button type="button" onClick={onReset} className="hidden text-xs font-semibold transition-opacity hover:opacity-60 sm:block" style={{ color: MUTED }}>
            Reset
          </button>

          
          <div className="relative sm:hidden">
            <button type="button" onClick={() => setMobileMenu(v => !v)} className="btn-outline border px-3 py-2 text-xs font-semibold" style={{ borderColor: BORD, color: DARK }}>
              ⋯
            </button>
            {mobileMenu && (<div className="absolute right-0 top-10 z-40 w-48 overflow-hidden border bg-white shadow-xl" style={{ borderColor: BORD }} onClick={() => setMobileMenu(false)}>
          ...
          {user && (<div className="ml-1 flex h-7 w-7 items-center justify-center border text-[11px] font-black" style={{ borderColor: DARK, color: DARK }} title={user.name}>
              {user.name?.[0]?.toUpperCase() || "U"}
            </div>)}
        </div>
      </header>

      {requireAuthForAi && !user && (<div className="shrink-0 border-b px-4 py-2.5 text-center text-xs sm:text-[13px]" style={{ borderColor: BORD, background: "#fffbeb", color: "#78350f" }}>
      ...
      <div className="flex shrink-0 border-b xl:hidden" style={{ borderColor: BORD, background: BG }}>
        {[["brief", "Brief"], ["diagram", "Diagram"], ["insights", "Insights"]].map(([id, label]) => (<button key={id} type="button" onClick={() => setMobileTab(id)} className="flex-1 py-2.5 text-xs font-semibold transition-colors" style={{
      ...
      <div className={`flex min-h-0 flex-1 flex-col gap-2 p-2 sm:gap-3 sm:p-3 xl:grid xl:gap-3 xl:p-3 ${showInsights ? "xl:grid-cols-[300px_minmax(0,1fr)_320px]" : "xl:grid-cols-[300px_minmax(0,1fr)]"}`}>
          ...
          <div className="scroll-surface flex-1 overflow-y-auto">
            <div className="space-y-5 px-4 py-4">
              <label className="block space-y-1.5">
                ...
                <Input as="textarea" rows={6} value={form.prompt} onChange={(e: ChangeEvent<HTMLTextAreaElement>) => onPromptChange(e.target.value)} className="resize-none bg-gray-50 text-sm" placeholder="Describe the product, users, and critical flows."/>
              </label>
              <div className="rounded-lg border border-amber-100 bg-amber-50/90 px-3 py-2.5">
                ...
                <button type="button" onClick={onLoadStressTest} className="mt-1 text-left text-xs font-semibold text-amber-900 underline-offset-2 hover:underline">
            ...
            ["region", "Region"],
            ["security", "Security"],
        ] as const).map(([k, l]) => (<ConstraintField key={k} label={l} value={form.constraints[k]} onChange={(v) => onConstraintChange(k, v)}/>))}
              </div>
              <div>
                <p className="mb-2.5 text-[11px] font-bold uppercase tracking-wide text-gray-400">Snapshot</p>
                <div className="grid grid-cols-2 gap-2">
                  {metrics.map((m) => (<div key={m.label} className="rounded-lg border border-gray-100 bg-gray-50 px-3 py-2.5">
              ...
              <button type="button" onClick={onGenerate} disabled={loading} className="btn-outline w-full border py-3 text-sm font-bold disabled:opacity-50 xl:hidden" style={{ borderColor: DARK, color: DARK }}>
        ...
        {showInsights && (<aside className={`min-h-0 overflow-hidden rounded-xl border border-gray-200 bg-white
            ${mobileTab === "insights"
                ? "flex flex-1 flex-col"
                : "hidden xl:flex xl:flex-col"}`}>
        ...
        {!showInsights && mobileTab === "insights" && (<div className="flex flex-1 flex-col items-center justify-center gap-3 xl:hidden">
            ...
            <button type="button" onClick={onToggleInsights} className="btn-outline mt-2 border px-6 py-2.5 text-sm font-semibold" style={{ borderColor: DARK, color: DARK }}>
              Enable insights
            </button>
          </div>)}
      </div>
...
export default function App() {
    ...
    const [design, setDesign] = useState<NormalizedDesign>(() => createFallbackDesign("Build a learning platform where students can watch live classes, message mentors, and get AI-generated study plans.", INITIAL_FORM.constraints));
    ...
    useEffect(() => {
        fetch(apiUrl("/api/public-config"))
            ...
            .then((c: {
            requireAuthForAi?: boolean;
        }) => setPublicConfig({ requireAuthForAi: Boolean(c.requireAuthForAi) }))
            .catch(() => { });
    }, []);
    useEffect(() => {
    ...
    const handleAuth = useCallback(async (mode: "signup" | "login", fields: {
        name?: string;
        email: string;
        password: string;
    }) => {
    ...
    const handleNewDesignStart = useCallback(({ prompt, constraints, meta }: {
        prompt: string;
        constraints: Record<string, string>;
        meta?: {
            domain?: string;
    ...
    const handleGenerate = useCallback(async () => {
        if (!form.prompt.trim()) {
            toast$("Add a product idea first.", "error");
            return;
        }
        setLoading(true);
        ...
        try {
            ...
            if (payload.usedFallback) {
                toast$("We could not use the AI for this run (check your API key or try again). A template design was returned instead.", "info");
            }
            else {
    ...
    const exportFile = useCallback(async (format: ExportFormat) => {
        if (!hasDesign) {
            toast$("Generate a design first.", "error");
            return;
        }
        setExporting(format);
        setShowExportMenu(false);
        if (format === "md") {
            try {
                downloadBlob(new Blob([generateMarkdown(design)], { type: "text/markdown;charset=utf-8" }), "system-design.md");
    ...
    return (<ErrorBoundary>
      ...
      {screen === "workspace" ? (<WorkspaceScreen form={form} design={design} loading={loading} exporting={exporting} hasDesign={hasDesign} showInsights={showInsights} showExportMenu={showExportMenu} metrics={metrics} user={user} designMeta={designMeta} generationMeta={generationMeta} requireAuthForAi={publicConfig.requireAuthForAi} onPromptChange={(v) => setForm((c) => ({ ...c, prompt: v }))} onConstraintChange={(k, v) => setForm((c) => ({ ...c, constraints: { ...c.constraints, [k]: v } }))} onLoadStressTest={() => { setForm(stressTestForm()); setShowInsights(true); }} onGenerate={handleGenerate} onReset={handleReset} onExport={exportFile} onToggleInsights={() => setShowInsights((v) => !v)} onToggleExportMenu={() => setShowExportMenu((v) => !v)} onCloseMenus={() => { setShowInsights(false); setShowExportMenu(false); }} onGoHome={() => setScreen(user ? "dashboard" : "landing")} onNewDesign={() => setScreen("newdesign")} onRequestSignIn={() => setScreen("auth")}/>) : screen === "newdesign" ? (<NewDesignScreen onStart={handleNewDesignStart} onBack={() => setScreen(user ? "dashboard" : "landing")}/>) : screen === "dashboard" ? (user ? (<DashboardScreen user={user} history={history} onNewDesign={handleNewDesign} onOpenHistory={handleOpenHistory} onLogout={handleLogout} onDeleteDesign={handleDeleteDesign}/>) : null) : screen === "auth" ? (<AuthScreen onAuth={handleAuth} onBack={() => setScreen("landing")} initialMode="login" onForgotPassword={() => setScreen("forgot-password")}/>) : screen === "forgot-password" ? (<ForgotPasswordScreen onBack={() => setScreen("auth")} onContinueWithCode={(c) => {
            ...
            }}/>) : screen === "onboarding" ? (<OnboardingScreen form={form} signup={signup} onPromptChange={(v) => setForm((c) => ({ ...c, prompt: v }))} onConstraintChange={(k, v) => setForm((c) => ({ ...c, constraints: { ...c.constraints, [k]: v } }))} onSignupChange={(k, v) => setSignup((c) => ({ ...c, [k]: v }))} onSubmit={handleSignupSubmit} onBack={() => setScreen("landing")}/>) : (<LandingScreen onGetStarted={() => setScreen("onboarding")} onSignIn={() => setScreen("auth")}/>)}
    </ErrorBoundary>);
}
