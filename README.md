...
This app only needs Postgres over **`DATABASE_URL`** (no Supabase client SDK in the server).

1. Create a project at [supabase.com](https://supabase.com/) (free tier is enough).
2. Open **Project Settings** → **Database**.
3. Under **Connection string**, choose **URI** and use the **Direct connection** (host `db.<project-ref>.supabase.co`, port **5432**).  
   Avoid the **Transaction pooler** (port 6543) for this service — the Node `pg` pool is a long‑lived server on Render and works best with the direct session.
4. Replace `[YOUR-PASSWORD]` in the string with your database password (reset it on the same page if needed).
5. In Render → your web service → **Environment**, set **`DATABASE_URL`** to that full URI (starts with `postgresql://` or `postgres://`). Do not commit it to git.
6. **Save** and trigger **Manual Deploy** (or push to `main` if auto-deploy is on). After the first successful boot, tables are created automatically by `server.ts`.

The server enables TLS for non-localhost URLs, which matches Supabase’s requirements.
...
4. In the service **Environment** tab, add **`DATABASE_URL`** (Supabase **Direct** URI — see [Supabase Postgres for DATABASE_URL](#supabase-postgres-for-database_url)) and **`GEMINI_API_KEY`**.  
